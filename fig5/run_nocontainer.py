...
def udf_skl_predict(iterator: Iterator[pd.Series]) -> Iterator[pd.Series]:
    ...
    f = open(path+stagepart+'pandas-predict.txt', 'a')
    f.write("NORMALFUNCTION CALLED")
    f.flush()
    ...
    for args in iterator:
        ...
        f.write("called predict\n")
        f.write(str(predictions.size))
...
resultfile = open("results_1nocontainer.txt", "a")
